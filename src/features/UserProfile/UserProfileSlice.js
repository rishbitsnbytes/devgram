...
export const getUserProfile = createAsyncThunk(
  ...
  async ({ authToken, username }, { rejectWithValue }) => {
    try {
      const { data } = await getUserProfileDetailsService(authToken, username);
      return data.user;
    } catch (error) {
...
const userProfileSlice = createSlice({
  ...
  reducers: {
    ...
    setUserProfileInitialState: (state, action) => {
      state.userProfile = {};
...
export const { removeUserProfile, setUserProfileInitialState } =
