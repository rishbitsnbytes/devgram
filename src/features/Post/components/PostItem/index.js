...
const PostItem = ({ post, location }) => {
  ...
  const handleDeletePost = async (event) => {
    ...
    event.preventDefault();
    try {
      const response = await dispatch(deletePost({ authToken, postId: _id }));
      if (response?.error) {
  ...
  const handleBookmarkStateChange = async (event) => {
    ...
    try {
      const response = isPostInBookmarks
        ? await dispatch(removeBookmarkedPost({ authToken, postId: _id }))
    ...
    <div className="post-item border dark:bg-slate-800 bg-gray-100 border-gray-300 dark:border-slate-500 flex flex-col p-4 w-full rounded-sm gap-6 shadow-sm max-w-[1080px]">
      <div
        className={`flex flex-row items-start justify-between gap-4 w-full rounded-sm ${
          location === "singlePost" ? "cursor-default" : "cursor-pointer"
        }`}
      ...
      >
        ...
        <div className="flex flex-col items-start justify-between w-full gap-4">
          <div className="flex flex-row gap-1 justify-between items-center flex-wrap w-full">
            ...
            <p className="text-xs text-grat-400 w-max">{formattedDate}</p>
          </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-row justify-between items-center">
          ...
          >
            ...
            {likeCount > 0 ? (
              <span className="like-count text-base">{likeCount}</span>
            ) : null}
          </button>
          <button
