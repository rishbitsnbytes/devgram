...
const EditProfileForm = () => {
  ...
  const { userProfile, userProfileLoading } = useSelector(getUserProfileState);
  ...
  const handleOnProfileImageChange = (event) => {
    ...
    fetch(url, {
      ...
      .catch((error) =>
        showToast("Failed to update image. Please try again later.", "error")
      );
  };
  ...
  const handleEditProfile = async (event) => {
    ...
    try {
      ...
      if (response?.error)
        throw new Error("Failed to edit profile. Please try again later.");
