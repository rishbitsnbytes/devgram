...
const Profile = () => {
  ...
  useEffect(() => {
    setDocumentTitle("Devgram | Profile");
    (async () => {
      try {
    ...
    (async () => {
      try {
        const response = await dispatch(getUserPosts({ authToken, username }));
        if (response.error)
          throw new Error(
    ...
    <section className="home p-8 px-0 md:px-8 border-0 md:border-l lg:border-r border-x-sky-400 flex flex-col items-center justify-start w-full gap-4">
      ...
      {sortedPosts?.length ? (
        ...
        <h3 className="text-lg md:text-2xl mt-3 text-green-600 font-semibold text-center">
          {username === authUsername ? "You" : "They"} are yet to share{" "}
          {username === authUsername ? "your" : "their"} musings about reading!
        </h3>
      )}
    </section>
  );
